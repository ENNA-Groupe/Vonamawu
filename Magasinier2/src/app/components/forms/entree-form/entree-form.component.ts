...
export class EntreeFormComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    if (this.produitsArray.filter(item=>item.produit_id===null||item.quantite===null).length>0) {
      this.alert.toast({ type: 'danger', message: 'Veuillez remplir toutes les cases!'});
