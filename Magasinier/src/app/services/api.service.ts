...
export class ApiService {
  ...
  getAll() {
    this.http.get<any[]>(this.url + 'data/magasinier/' + this.user.id, this.options).subscribe(
      (res) => {
        console.log(res);
  ...
  login(data: any) {
    return new Promise(
      (resolve, reject) => {
        this.http.post<any>(this.url + 'magasinierLogin', data, this.options).subscribe(
          (res: any) => {
  ...
  getData() {
    ...
    this.getRoute('data', 'magasinier/' + this.user.id + '/' + this.dataService.lastId).subscribe(
      (res: any) => {
        console.log(res);
