...
export class ApiService {
  ...
  getAll() {
    this.http.get<any[]>(this.url + 'data/vendeur/' + this.user.id, this.options).subscribe(
      (res) => {
        console.log(res);
  ...
  login(data: any) {
    return new Promise(
      (resolve, reject) => {
        this.http.post<any>(this.url + 'vendeurLogin', data, this.options).subscribe(
          (res: any) => {
  ...
  getData() {
    ...
    this.getRoute('data', 'vendeur/' + this.user.id + '/' + this.dataService.lastId).subscribe(
      (res: any) => {
        console.log(res);
