...
export class EntreeProfilComponent implements OnInit, OnDestroy {
      ...
      item => {
        ...
        if (item) {
          if (this.searchService.menu === 'GM') {
            ...
            this.titre = "Entree de " + item.nom + " numero " + item.numero;
            this.operation = item;
            this.entreeData.showMaison(item.id).subscribe(
              (prod: any) => {
                console.log(prod);
                this.produitsArray = prod.produits;
              }
  ...
  onAnnuler() {
    this.entreeData.validate({ id: this.operation.id, numero: this.operation.numero, fournisseur_nom: this.operation.nom, response: 2 });
    this.onClose();
  }
}
