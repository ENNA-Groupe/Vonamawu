...
export class MagasiniersComponent implements OnInit {
  ...
  onActivate(){
    if (window.confirm("Voulez vous activer tous les magasiniers?")) {
      this.magasinierData.activate({global:true, isActive:true});
    }
   
  }

  onDesactivate(){
    if (window.confirm("Voulez vous desactiver tous les magasiniers?")) {
      this.magasinierData.activate({global:true, isActive:false});
    }
  }
