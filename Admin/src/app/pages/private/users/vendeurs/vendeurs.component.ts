...
export class VendeursComponent implements OnInit {
  ...
  onActivate(){
    if (window.confirm("Voulez vous activer tous les vendeurs?")) {
      this.vendeurData.activate({global:true, isActive:true});
    }
   
  }

  onDesactivate(){
    if (window.confirm("Voulez vous desactiver tous les vendeurs?")) {
      this.vendeurData.activate({global:true, isActive:false});
    }
  }
